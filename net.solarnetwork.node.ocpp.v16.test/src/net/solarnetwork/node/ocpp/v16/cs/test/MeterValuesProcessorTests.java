...
package net.solarnetwork.node.ocpp.v16.cs.test;
...
import net.solarnetwork.node.ocpp.v16.cs.MeterValuesProcessor;
...
public class MeterValuesProcessorTests {
	...
	public void process_ok() throws InterruptedException {
		...
		Capture<Iterable<net.solarnetwork.node.ocpp.domain.SampledValue>> readingsCaptor = new Capture<>();
		...
		List<net.solarnetwork.node.ocpp.domain.SampledValue> txData = StreamSupport
		...
		for ( int i = 0; i < 2; i++ ) {
			net.solarnetwork.node.ocpp.domain.SampledValue sve = txData.get(i);
			...
			assertThat("Reading context translated " + i, sve.getContext(),
					equalTo(net.solarnetwork.node.ocpp.domain.ReadingContext.SamplePeriodic));
			assertThat("Location translated " + i, sve.getLocation(),
					equalTo(net.solarnetwork.node.ocpp.domain.Location.Outlet));
			if ( i == 0 ) {
				assertThat("Measurand translated 0", sve.getMeasurand(),
						equalTo(net.solarnetwork.node.ocpp.domain.Measurand.EnergyActiveImportRegister));
				assertThat("Unit trarnslated 0 ", sve.getUnit(),
						equalTo(net.solarnetwork.node.ocpp.domain.UnitOfMeasure.Wh));
				assertThat("Value copied 0", sve.getValue(), equalTo("1234"));
			} else {
				assertThat("Measurand translated 1", sve.getMeasurand(),
						equalTo(net.solarnetwork.node.ocpp.domain.Measurand.PowerActiveImport));
				assertThat("Unit trarnslated 1 ", sve.getUnit(),
						equalTo(net.solarnetwork.node.ocpp.domain.UnitOfMeasure.W));
