...
public class JdbcAuthorizationDaoTests extends AbstractNodeTransactionalTest {
	...
	private Authorization createTestAuthorization(String vendor, String model) {
		Authorization cp = new Authorization(UUID.randomUUID().toString().substring(0, 20),
				Instant.now());
		cp.setEnabled(true);

		AuthorizationInfo info = new AuthorizationInfo(cp.getId(), AuthorizationStatus.Unknown,
				cp.getCreated().plus(1, ChronoUnit.HOURS),
	...
	@Test
	public void insert() {
		Authorization cp = createTestAuthorization("foo", "bar");
	...
	public void update() {
		insert();
		Authorization cp = dao.get(last.getId());
		cp.setInfo(cp.getInfo().toBuilder().withStatus(AuthorizationStatus.Accepted)
				.withParentIdTag(null).build());
		String pk = dao.save(cp);
		assertThat("PK unchanged", pk, equalTo(cp.getId()));

		Authorization entity = dao.get(pk);
		assertThat("Status updated", entity.getInfo().getStatus(),
				equalTo(AuthorizationStatus.Accepted));
		assertThat("Vendor updated", entity.getInfo().getParentIdTag(),
				equalTo(cp.getInfo().getParentIdTag()));
	}
