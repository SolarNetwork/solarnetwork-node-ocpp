...
public class JdbcSystemUserDao extends BaseJdbcGenericDao<SystemUser, Long> implements SystemUserDao {
	...
	public JdbcSystemUserDao() {
		super(SystemUser.class, Long.class, SystemUserRowMapper.INSTANCE, TABLE_NAME_TEMPALTE,
				TABLE_NAME, VERSION);
	...
	@Override
	protected @Nullable SystemUser findFirst(String sql, Object... parameters) {
		List<SystemUser> results = jdbcTemplate().query(sql, SYSTEM_USER_EXTRACTOR, parameters);
		return (results != null && !results.isEmpty() ? results.get(0) : null);
	}

	@Override
	public Collection<SystemUser> getAll(@Nullable List<SortDescriptor> sorts) {
		List<SystemUser> result = jdbcTemplate().query(querySql(SQL_FIND_ALL, sorts),
				SYSTEM_USER_EXTRACTOR);
		return (result != null ? result : List.of());
	}

	@Override
	protected @Nullable Long storeDomainObject(SystemUser obj, String sqlInsert) {
	...
	private void insertAllowedChargePoints(Long userId, SystemUser obj) {
		...
		jdbcTemplate().batchUpdate(getSqlResource(SqlResource.InsertAllowedChargePoint.getResource()),
	...
	private void updateAllowedChargePoints(SystemUser obj) {
		...
		for ( ListIterator<String> itr = chargePointIds.listIterator(); itr.hasNext(); ) {
			...
			if ( count < 1 ) {
				jdbcTemplate().update(getSqlResource(SqlResource.InsertAllowedChargePoint.getResource()),
						new PreparedStatementSetter() {
		...
		jdbcTemplate().update(getSqlResource(SqlResource.DeleteAllowedChargePointsOver.getResource()),
	...
	public static final class SystemUserRowMapper implements RowMapper<SystemUser> {
		...
		/** A standard instance. */
		public static final RowMapper<SystemUser> INSTANCE = new SystemUserRowMapper();
	...
	public static final class AllowedChargePointRowMapper implements RowMapper<String> {
		...
		@Override
		public @Nullable String mapRow(ResultSet rs, int rowNum) throws SQLException {
			rs.getLong(1 + offset);
			if ( rs.wasNull() ) {
				return null;
			}
	...
	public static final class SystemUserResultSetExtractor
		 ...
		 */
		public SystemUserResultSetExtractor(RowMapper<SystemUser> mainRowMapper) {
			super();
		...
		public List<SystemUser> extractData(ResultSet rs) throws SQLException, DataAccessException {
			...
			while ( rs.next() ) {
				rowNum++;
				SystemUser rowMainEntity = nonnull(mainRowMapper.mapRow(rs, rowNum), "SystemUser");
				SystemUser entity = map.get(rowMainEntity.id());
				if ( entity == null ) {
					// new main entity row
